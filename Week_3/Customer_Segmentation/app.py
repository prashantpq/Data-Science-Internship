...
with st.form("my_form"):
    ...
    Education_Level=st.number_input(label='Education Level',step=0.01,format="%.2f")
    Marital_Status=st.number_input(label='Marital Status',step=0.01,format="%.6f")
    Income_Category=st.number_input(label='Income Category',step=0.01,format="%.6f")
    Card_Category=st.number_input(label='Card Category',step=0.1,format="%.6f")
    Months_on_book=st.number_input(label='Months on book',step=0.1,format="%.6f")
    Total_Relationship_Count=st.number_input(label='Total Relationship Count',step=0.1,format="%.6f")
    Months_Inactive_12_mon=st.number_input(label='Months Inactive 12_mon',step=1)
    Contacts_Count_12_mon=st.number_input(label='Contacts Count 12 mon',step=1)
    ...
    Total_Amt_Chng_Q4_Q1=st.number_input(label='Total Amt Chng Q4 Q1',step=0.01,format="%.6f")
    Total_Trans_Amt=st.number_input(label='Total Trans Amt',step=1)
    Total_Trans_Ct=st.number_input(label='Total Trans Ct',step=1)
    Total_Ct_Chng_Q4_Q1=st.number_input(label='Total Ct Chng Q4 Q1',step=1)
    Avg_Utilization_Ratio=st.number_input(label='Avg Utilization Ratio',step=1)
